...
<template>
  <glass-card radius="1.1rem" pad="0.7rem 0.95rem" class="rise del-host">
    <div class="flex items-center gap-3">
      <button
        ...
        @click="toggle($event)"
      >
        <us-icon v-if="item.done" name="Check" :size="14" stroke-width="3" />
      </button>
      ...
      <span class="chip" :class="{ 'chip-outline': item.assignee !== me }">{{ item.assignee === me ? "Me" : item.assignee }}</span>
